...
export default function BarChart() {
    ...
    useEffect(() => {
        ...
        if (categorical.includes(attribute)) {
            ...
            male_data.forEach((row) => (
                domain[row[attribute]]["male"] = domain[row[attribute]]["male"] + 1
            ))

            female_data.forEach((row) => (
                domain[row[attribute]]["female"] = domain[row[attribute]]["female"] + 1
            ))
            ...
            male_data.forEach((d) => {
                const binIndex = bins.findIndex((bin) => d[attribute] >= bin.x0 && d[attribute] < bin.x1);
                if (binIndex != -1)
